...
spacer
echo "  ${BOLD}One API. Every GPU. Any Region.${RESET}"
spacer
...
# ── Pre-Demo Cleanup ─────────────────────────────────────────────────────────
# Delete leftover jobs from previous demo runs. Volcano's webhook rejects
# updates to existing jobs, so we need a clean slate. This runs silently
# to avoid cluttering the demo output.
...
countdown "Waiting for Slurm job to start" "$WAIT_FOR_POD"
...
run_cmd "kubectl logs job/slurm-test -n gco-jobs --tail=20 2>/dev/null || echo '  (logs not yet available — job still initializing)'"
...
countdown "Waiting for job to run" "$WAIT_FOR_POD"
...
countdown "Waiting for job completion" "$WAIT_FOR_POD"
run_cmd "kubectl logs job/fsx-lustre-example -n gco-jobs --tail=30 2>/dev/null || echo '  (job still running — logs will appear on completion)'"
...
countdown "Waiting for completion" "$WAIT_FOR_POD"

highlight "Valkey job output"
run_cmd "kubectl logs job/valkey-cache-example -n gco-jobs --tail=20 2>/dev/null || echo '  (job still running...)'"
...
run_cmd "kubectl logs job/efs-output-example -n gco-jobs --tail=15 2>/dev/null || echo '  (job still running...)'"
...
run_cmd "gco files download efs-output-example /tmp/gco-demo-results -r $REGION 2>/dev/null && cat /tmp/gco-demo-results/results.json 2>/dev/null || echo '  (download will work once job completes and EFS syncs)'"
