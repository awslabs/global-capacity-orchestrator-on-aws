...
| `test_inference.py` | `InferenceManager` deploy/list/describe/scale/delete (CLI surface), `InferenceMonitor` reconciliation loop (deployment / service / ingress lifecycle, status reporting, leader-election bootstrap). |
| `test_inference_extended.py` | `InferenceMonitor` long tail — `_try_acquire_lease` leader-election (renew, claim-when-empty, claim-when-None, not-leader), HPA creation, `_create_deployment` body matrix (model_source S3 sync init container, model_path volumes, env vars, command/args, zero-GPU, custom node selector, custom resource requests), the start/stop lifecycle, the monitor's `main()` entry point, and `InferenceManager.add_region` / `remove_region`. Plus `_resolve_image_for_region` per-region URI selection — happy path picks the regional ECR replica from `spec["region_image_uris"][self.region]`, falls back to the flat `spec["image"]` when the map lacks the active region, when no map is present, when the map is malformed (list instead of dict), and when the per-region value is `None` or empty. End-to-end check via `_create_deployment` confirms the wired-in selection makes it into the materialised pod spec. |
...
| `test_inference_health_watchdog.py` | Inference endpoint health watchdog — recovery-from-failure logic, status synchronisation between Kubernetes and DynamoDB, restart-on-CrashLoopBackOff orchestration. |
...
| `test_lambda_image_lookup.py` | Tests for `lambda/image-lookup/handler.py` — the ECR adopt-or-create custom resource. Covers `_describe_repository` (typed `RepositoryNotFoundException`, generic `ClientError` shape with `RepositoryNotFoundException` code, propagation of other errors, empty-list short-circuit), `_create_repository` (project-standard `MUTABLE` + `scanOnPush=true`), `_apply_lifecycle_policy` (no-op on empty / whitespace / `None`, JSON-validation failure before the API call), `_has_retain_tag` (case-insensitive match, swallowed `list_tags_for_resource` errors, `None` tag list), `_delete_all_images` (paginated digest collection across multiple `describe_images` pages, chunking into 100-id `batch_delete_image` calls, skipping entries without a digest, empty repo zero-call short-circuit), every `lambda_handler` Create/Update branch (fresh create, adopt-when-present, lifecycle policy after create, fallback when ARN is missing), every Delete branch (idempotent absence, retain-tag short-circuit, removal-policy retain, destroy with `EmptyOnDelete=True` purging then deleting, destroy with `EmptyOnDelete=False` skipping the purge), and dispatcher errors (unsupported `RequestType`, missing `RequestType`, `None` `ResourceProperties`). |
...
| `test_mcp_transforms.py` | FastMCP transform behaviour — `ResourcesAsTools` round-trip, BM25 / Regex / Code Mode / `off` selection via `GCO_MCP_TOOL_SEARCH`, default-env fallback to BM25, unknown-value fallback, always-visible entry-point set survives catalog replacement, Code Mode discovery-tool order (`[GetTags, Search, GetSchemas]`), `MontySandboxProvider` limits via `GCO_MCP_CODE_MODE_MAX_DURATION_SECS` / `GCO_MCP_CODE_MODE_MAX_MEMORY` (defaults + env overrides + invalid-value fallback), startup audit log carries `code_mode_experimental: true` under Code Mode. |
...
| `test_mcp_tasks.py` | FastMCP background-task tooling — `_run_long_task` lifecycle (drain stdout/stderr, increment progress on CFN `*_COMPLETE` lines), cancellation with `SIGTERM` → 10s grace → `SIGKILL`, partial-CloudFormation-state disclaimer in cancelled stack ops, path-traversal rejection in argv. Plus the deploy/destroy gating tests (`deploy_stack` / `deploy_all` / `bootstrap_cdk` absent without `GCO_ENABLE_INFRASTRUCTURE_DEPLOY`, `destroy_stack` / `destroy_all` absent without `GCO_ENABLE_INFRASTRUCTURE_DESTROY`), argv kick-off tests, and the audit-log task-id correlation test. |
| `test_mcp_destructive_gating.py` | Destructive-flag gating — `delete_job` and `delete_inference` (the intentional backward-compat break) absent by default, present under `GCO_ENABLE_DESTRUCTIVE_OPERATIONS=true`. Same coverage for `delete_template` / `delete_webhook` / `delete_model` / `delete_nodepool` / `analytics_user_remove` / `cancel_queue_job`, plus the `data-upload` `models_upload` tool under `GCO_ENABLE_MODEL_UPLOAD`. The umbrella-flag test confirms `GCO_ENABLE_ALL_TOOLS=true` registers every gated tool name in one shot, even with every per-tool flag explicitly cleared. Argv translation tests confirm each destructive tool builds the expected CLI invocation (`-y`, positional name, optional region/cluster). |
...
| File | Description |
|------|-------------|
| `test_no_python_315_deprecation_surface.py` | Walks the production tree (`mcp/`, `cli/`, `gco/`, `lambda/`, `tests/`, `dockerfiles/`, plus the project `README.md`) and fails if any of the patterns Python 3.15 soft-deprecates re-appear: `collections.abc.ByteString`, `typing.ByteString`, `typing.no_type_check_decorator`, `cProfile` (import), `glob.glob0` / `glob.glob1`, `platform.java_ver`, `load_module` / `find_module` / `zipimporter`, `NamedTuple("Name", field=type)` keyword-argument syntax, `TypedDict("Name")` zero-field syntax, and bare `re.match(` calls outside the two intentional carve-outs (`tests/test_integration.py` and `cli/kubectl_helpers.py`). Failure is structured as `path:line: [pattern] line-content`. |
| `test_no_spec_references.py` | Walks `mcp/`, `cli/`, `gco/`, `lambda/`, `tests/`, `dockerfiles/`, `examples/`, `docs/`, `scripts/`, plus the project root README family, and fails if any prohibited spec / planning prose substring appears. The substring set covers filenames (`requirements.md`, `design.md`, `tasks.md`, `bugfix.md`) plus prose phrases (`per the requirements`, `per the design`, `per the spec`, `as the spec says`, `see the requirements doc`, `see the design doc`, `see the tasks doc`). The test self-excludes via `Path(__file__)` so its own string literals don't trip the check. |
